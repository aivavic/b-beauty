<?
	if($act=="cat" || $act=="akcii" || $act=="novinki" || $act=="bestsellers" || $act=="search")
	{
        ...
        if (isset($_REQUEST['changebrend']) && $_REQUEST['changebrend']==1) $addsql = " AND `brendid`=".(int)$_REQUEST['brendid']; else $addsql = '';
        if (isset($_REQUEST['changesize']) && $_REQUEST['size']!='--') $addsize = " AND CONCAT(';',`razmer`,';') LIKE '%;".$_REQUEST['size'].";%'"; else $addsize='';
        if (!isset($_REQUEST['changeorder']) || $_REQUEST['changeorder']==1) $ord = "DESC"; else $ord="ASC";
...
//		}
       ...
       foreach ($products AS $k=>$val)
       {
        ...
        if ($act == 'cat' && $id!=0)
        {
            ...
            $_logic['sizes'] = array_unique($sizes);
            sort($_logic['sizes']);
            //debug($_logic['sizes']);
