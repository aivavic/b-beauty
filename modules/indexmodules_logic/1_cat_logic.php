<?
	if($act=="cat" || $act=="akcii" || $act=="novinki" || $act=="bestsellers" || $act=="search")
	{
        ...
        if (isset($_SESSION['currbrend'][$id])) $addsql = " AND `brendid`=".(int)$_SESSION['currbrend'][$id]; else $addsql = '';
        if (isset($_SESSION['size'])) $addsize = " AND CONCAT(';',`razmer`,';') LIKE '%;".$_SESSION['size'].";%'"; else $addsize='';
		...
		if($act=="cat"&&$id!=0)
		{
			...
			$sql2 = "SELECT COUNT(id) AS ccc FROM $par->objectstable WHERE categid IN $instr AND hide=0 $addsql ";
