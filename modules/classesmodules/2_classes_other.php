	...
	function GetAddStrH($h,$fname,$size=null)
{
	...
	function GetPager($sql,$linkstr,$objperpage=0,$addstr='',$startmode=0)
	{
		...
		if($allpages>1)
		{
			if($currpage>1) $pagerarr[] = Array('url'=>$linkstr.(($currpage-2)*$tovarsinpage).$addstr , 'value'=>'предыдущая');
			...
			if($currpage<$allpages) $pagerarr[] = Array('url'=>$linkstr.(($currpage)*$tovarsinpage).$addstr , 'value'=>'следующая');
		}
		return $pagerarr;
	}
	...
	function GetSeoUrl($act,$id,$line=null,$needlang=null,$subact="")
	{
			...
			if($needact==$act)
			{
				...
				else
				{
					...
					if($line)
					{
						...
						if(trim($line['url'.$t_langadd])=='')
						{
							if(trim($line['seo'.$t_langadd])=='') $returnurl = '/'.$urlprefix.'/'.$line['id'].$t_urllangadd;
							else
							{
		...
		if(isset($start) && $start!=0) $returnurl.='/start/'.$start;
