...
*/
			...
			'brendid2' =>Array('fieldtype'=>'text',   /*для multiselect*/ 'visualtype'=>'multiselect',  'selecttable'=>'topmenu', 'selecttablefield'=>'title', 'selectorderfield'=>'id ASC', 'selectmaxlevel'=>1 , 'height'=>200, /*end - для multiselect*/          'fieldhint'=>'MBrend', 'multilang'=>false, 'metod'=>'MetodFormStandart', 'metodedit'=>'MetodEditStandart', 'metodlist'=>'MetodListStandart', 'tdname'=>'МультиКатегория'),
...
//				$r2 = uploadify($this->id,'smfotor',1);
		...
		function DelAllFotos($itemid)
		{
			...
			for($p=0;$p<count($a);$p++)
			{
				...
				if($field_key!='')
				{
					if($this->fields[$field_key]['visualtype']=='image') $this->DelOneFoto($itemid,$field_key);
				}
			}
		}
		...
		function DelAllGallery($itemid)
		{
			...
			for($p=0;$p<count($a);$p++)
			{
				...
				if($field_key!='')
				{
					if($this->fields[$field_key]['visualtype']=='gallery') $this->DelOneGallery($itemid,$field_key);
				}
			}
		}
			...
			function DelAllFiles($itemid)
		{
			...
			for($p=0;$p<count($a);$p++)
			{
				...
				if($field_key!='')
				{
					if($this->fields[$field_key]['visualtype']=='file') $this->DelOneFile($itemid,$field_key);
				}
			}
		}
