...
<div class="items">
	<?
		...
		while($line=mysql_fetch_array($res,MYSQL_ASSOC)){
			...
			echo '<div class="item" id="'.$line['id'].'_'.$line['prior'].'"><span class="controls"><input type="text" name="name" id="'.$line['id'].'" value="'.$line['title'].'"><span class="del" onclick="if(confirm(\'Удалить???\')) del(this,'.$line['id'].')"></span></span><div class="img"><img src="'.$fname.$line['id'].'.jpg" alt="" /></div></div>';
/*						
...
*/			
	...
	?>
<script>
var imgOrder = '';var titles = new Array();
...
<?
	...
	if ($act=='updategal'){
		...
		foreach($old as $key=>$item){
			$item=explode('_',$item);$item=$item[1];
			$n=explode('_',$new[$key]);$n=$n[0];
